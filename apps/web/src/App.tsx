...
function App() {
  ...
  async function loadEvalRuns() {
    ...
    try {
      ...
      const text = loadError instanceof Error ? loadError.message : 'Failed to load eval runs.'
      setEvalsError(text)
    } finally {
  ...
  async function loadEvalRunDetail(evalRunId: number) {
    ...
    try {
      ...
      const text = loadError instanceof Error ? loadError.message : 'Failed to load eval run detail.'
      setEvalsError(text)
    } finally {
      setEvalsLoading(false)
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(submitError instanceof Error ? submitError.message : 'Unexpected error while sending request.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  async function submitStream(requestPayload: {
    ...
    setMessages((prev) => [
      ...prev,
      { id: assistantId, role: 'assistant', text: '', meta: { citations: [], ragUsed: false, mode: 'stream' } },
    ])
    ...
    function handleSseBlock(block: string) {
      ...
      if (currentEvent === 'start') {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === assistantId ? { ...msg, meta: { ...msg.meta, provider: start.provider, model: start.model, mode: 'stream' } } : msg,
          ),
        )
        return
      }
      if (currentEvent === 'token') {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === assistantId ? { ...msg, text: `${msg.text}${msg.text ? ' ' : ''}${token.text}` } : msg,
          ),
        )
        return
      }
      if (currentEvent === 'end') {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === assistantId
              ? { ...msg, text: end.answer, meta: { ...msg.meta, latencyMs: end.latency_ms, costUsd: end.cost_usd, mode: 'stream' } }
              : msg,
          ),
        )
      }
    ...
    <div className="app-shell">
      <aside className="control-panel" aria-label="Controls">
        <div className="panel-card">
          ...
          <h1>Iteration 13 Frontend</h1>
          <p className="panel-copy">
            Chat + eval dashboard in one workspace. Use the tabs below to switch between runtime testing and eval review.
          </p>
          <div className="tab-switch" role="tablist" aria-label="Workspace tabs">
            <button type="button" role="tab" aria-selected={view === 'chat'} className={view === 'chat' ? 'tab active' : 'tab'} onClick={() => setView('chat')}>
              Chat
            </button>
            <button type="button" role="tab" aria-selected={view === 'evals'} className={view === 'evals' ? 'tab active' : 'tab'} onClick={() => setView('evals')}>
              Evals
            </button>
          </div>
        </div>
        ...
        {view === 'chat' ? (
          <div className="panel-card stack">
            ...
            <label>
              ...
              <input value={model} onChange={(e) => setModel(e.target.value)} placeholder={providerPlaceholder} />
            </label>
            <label>
              <span>Mode</span>
            ...
            <div className="toggle-grid">
              <label className="toggle"><input type="checkbox" checked={rag} onChange={(e) => setRag(e.target.checked)} /><span>RAG</span></label>
              <label className="toggle"><input type="checkbox" checked={debug} onChange={(e) => setDebug(e.target.checked)} /><span>Debug</span></label>
            </div>
            <label>
              <span>Top K</span>
              <input type="number" min={1} max={10} value={topK} onChange={(e) => setTopK(Number(e.target.value))} disabled={!rag} />
            </label>
            {requestId ? (
              <div className="request-id-box"><span>Last Request ID</span><code>{requestId}</code></div>
            ) : null}
          </div>
        ) : (
          <div className="panel-card stack">
            <div className="row-between">
              ...
              <button type="button" className="ghost-button" onClick={() => void loadEvalRuns()}>
                Refresh
              </button>
            </div>
            {evalsLoading && evalRuns.length === 0 ? <p className="hint">Loading eval runs...</p> : null}
              ...
              {evalRuns.map((run) => {
                ...
                return (
                  <button
                    key={run.id}
                    type="button"
                    className={selectedEvalRunId === run.id ? 'run-item active' : 'run-item'}
        ...
        {view === 'chat' ? (
          <>
            <header className="chat-header">
              ...
              <button type="button" className="ghost-button" onClick={() => { setMessages([]); setError(null) }}>
                Clear
              </button>
            </header>
            <div className="chat-log" ref={scrollRef}>
              {messages.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p className="hint">Tip: Keep <strong>RAG</strong> and <strong>Debug</strong> enabled to see citations and retrieved chunks.</p>
                </div>
              ) : (
                messages.map((message) => (
                  <article key={message.id} className={`message-card ${message.role === 'assistant' ? 'assistant' : 'user'}`}>
                    <header className="message-meta">
                      <span className="role-tag">{message.role}</span>
                      {message.meta?.provider ? <span className="meta-pill">{message.meta.provider} · {message.meta.model}</span> : null}
                      {typeof message.meta?.latencyMs === 'number' ? <span className="meta-pill">{message.meta.latencyMs} ms</span> : null}
                      {typeof message.meta?.costUsd === 'number' ? <span className="meta-pill">${message.meta.costUsd.toFixed(4)}</span> : null}
                    ...
                    {message.meta?.citations && message.meta.citations.length > 0 ? (
                      <section className="subpanel">
                        <h3>Citations</h3>
                        <ul>{message.meta.citations.map((citation) => <li key={citation}><code>{citation}</code></li>)}</ul>
                      </section>
                    ) : null}
                          ...
                          {message.meta.retrievedChunks.map((chunk) => (
                            <article key={chunk.chunk_id} className="chunk-card">
                              <div className="chunk-card-header">
                                ...
                                <span>chunk {chunk.chunk_index} · score {chunk.score.toFixed(3)}</span>
                              </div>
                              <p>{chunk.content}</p>
                            </article>
                          ))}
            ...
            <form className="composer" onSubmit={handleSubmit}>
              ...
              <textarea id="message-input" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask something..." rows={4} disabled={isLoading} />
              <div className="composer-footer">
                {error ? <p className="error-text">{error}</p> : <span className="hint">Ready</span>}
                <button type="submit" disabled={isLoading || !input.trim()}>{isLoading ? 'Sending...' : mode === 'stream' ? 'Send (SSE)' : 'Send'}</button>
              </div>
            </form>
          </>
        ) : (
          <>
            ...
            <div className="chat-log eval-dashboard">
              {evalsError ? <p className="error-text">{evalsError}</p> : null}
              {!evalRunDetail && !evalsLoading ? <p className="hint">Select an eval run to inspect details.</p> : null}
              {evalRunDetail ? (
                <div className="eval-detail">
                  <section className="subpanel eval-summary-grid">
                    <div className="metric-card"><span>Run ID</span><strong>#{evalRunDetail.run.id}</strong></div>
                    <div className="metric-card"><span>Dataset</span><strong>{evalRunDetail.run.dataset_name}</strong></div>
                    <div className="metric-card"><span>Pass</span><strong>{evalRunDetail.run.passed_cases}/{evalRunDetail.run.total_cases}</strong></div>
                    <div className="metric-card"><span>Latency Avg</span><strong>{evalRunDetail.run.avg_latency_ms ?? 0} ms</strong></div>
                  </section>
                  <section className="subpanel">
                    <h3>Cases</h3>
                        ...
                        <tbody>
                          {evalRunDetail.cases.map((row) => (
                            <tr key={row.id}>
                              ...
                              <td>
                                <span className={row.passed ? 'status-pass' : 'status-fail'}>
