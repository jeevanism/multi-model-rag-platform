...
class GrokProvider(LLMProvider):
    def __init__(self, model: str = "grok-3-mini", mode: str = "stub") -> None:
    ...
    def _use_stub(self) -> bool:
        if self.mode == "stub":
            return True
        if self.mode == "auto":
            return not bool(os.getenv("GROK_API_KEY"))
        if self.mode == "real":
            return False
    ...
    def _generate_real(self, prompt: str) -> LLMResponse:
        ...
        if not api_key:
            raise RuntimeError("GROK_API_KEY is required when LLM_PROVIDER_MODE=real.")
        ...
        try:
            ...
            raise RuntimeError(
                "openai package is required for real Grok calls (OpenAI-compatible client). "
